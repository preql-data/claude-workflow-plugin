...
4. **The F7 completion contract** — the specialist's structured return payload with `task_id`, `files_changed`, `tests_added`, `decisions`, `blockers`, `llm_observations`, `context_coverage`. QA-specific extensions may be present when the specialist was QA itself; the base seven are mandatory.
...
- **`context_coverage` is mandatory and substantive, on the same three-way taxonomy.** Default criterion C8 fails when the field is empty, when it is boilerplate ("read the relevant files", "full context gathered"), or when its content is detached from the actual work — sources named that the diff plainly does not depend on, or the file carrying the diff's central change missing from the list. The shape is what-I-read / what-I-deliberately-skipped-and-why / the-largest-remaining-unknown; a coverage note with no deliberate omission in it is boilerplate by construction, because there is always one. Grade it against the diff in front of you, not against its own confidence.
...
4. Read the F7 completion contract. Cross-check `files_changed` against the diff (they should match); cross-check `decisions` against the SPEC (the calls should be coherent with the brief); read `llm_observations` for substantive content; read `context_coverage` and cross-check the sources it names against the diff — a coverage note that omits the file the central change lives in, or names sources the change does not depend on, fails C8 the same way an empty one does.
